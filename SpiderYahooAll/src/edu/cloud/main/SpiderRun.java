...
import edu.cloud.run.*;


public class SpiderRun {
    public static void main(String argsp[]){
    //爬取yahoopipes上的四个字段：pipeid、author、date、clonenum
    ...
    System.setProperty("http.proxyPort", "8087"); //goagent端口 ,代理访问       
     HtmlParser html = new HtmlParser();
   	 ...
   	 int i = 101;
   	 while(html.getHtmlContent("http://pipes.yahoo.com/pipes/pipes.popular?page="+i,"utf-8")=="successful"){
   		 html.getHtmlContent("http://pipes.yahoo.com/pipes/pipes.popular?page="+ i,"utf-8");
   	 ...
   	 for(int j = 0; j<idFormats.length; j++){
	  	...
	  	int k = 2;
	  	while(html.getHtmlContent("http://pipes.yahoo.com/pipes/search?page="+ k +"&r=format%3A"+idFormats[j],"utf-8")=="successful"){
	  		html.getHtmlContent("http://pipes.yahoo.com/pipes/search?page="+ k +"&r=format%3A"+idFormats[j],"utf-8");
	  		System.out.println("请求页面是formats的"+idFormats[j]+"第"+k+"页");
	   		 if(k == 50 ||k == 101){
	   			 try {
					Thread.sleep(300000);
				} catch (InterruptedException e) {
   	 ...
   	 for(int j = 0; j<idTags.length; j++){
	  	...
	  	int k = 2;
	  	while(html.getHtmlContent("http://pipes.yahoo.com/pipes/search?page="+ k +"&r=tag%3A"+idTags[j],"utf-8")=="successful"){
	  		html.getHtmlContent("http://pipes.yahoo.com/pipes/search?page="+ k +"&r=tag%3A"+idTags[j],"utf-8");
	  		System.out.println("请求页面是tags的"+idTags[j]+"第"+k+"页");
	   		 if(k == 50 ||k == 101){
	   			 try {
					Thread.sleep(300000);
				} catch (InterruptedException e) {
   	 ...
   	 for(int j = 0; j<idSources.length; j++){
	  	...
	  	int k = 2;
	  	while(html.getHtmlContent("http://pipes.yahoo.com/pipes/search?page="+ k +"&r=source%3A"+idSources[j],"utf-8")=="successful"){
   	 ...
   	 for(int j = 0; j<idModules.length; j++){
	  	...
	  	int k = 2;
	  	while(html.getHtmlContent("http://pipes.yahoo.com/pipes/search?page="+ k +"&r=module%3A"+idModules[j],"utf-8")=="successful"){
	  		html.getHtmlContent("http://pipes.yahoo.com/pipes/search?page="+ k +"&r=module%3A"+idModules[j],"utf-8");
